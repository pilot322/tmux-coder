...
func newServerWithGit(git *stubGit) *http.ServeMux {
	...
	create := usecase.NewCreateProject(state.Projects(), state.Sessions(), gw, state, state.Config(), obs.Nop())
	...
	agentEvent := usecase.NewAgentEvent(state.Agents(), state, obs.Nop())
